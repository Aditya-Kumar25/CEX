            ...
            <div className="w-full flex justify-center mt-2">
              <GoogleLogin
                ...
                onSuccess={async (credentialResponse) => {
                  console.log("[Google Auth Login] onSuccess triggered");
                  const cred = credentialResponse.credential || "";
                  console.log(`[Google Auth Login] Token length: ${cred.length}, starting with: ${cred.substring(0, 30)}...`);
                  if (cred) {
                    try {
                      setLoading(true);
                      setError(null);
                      await loginGoogle(cred);
                      console.log("[Google Auth Login] Authentication successful, navigating to dashboard");
                      navigate("/dashboard");
                    } catch (err) {
                      console.error("[Google Auth Login] Backend authentication failed:", err);
                ...
                onError={() => {
                  console.error("[Google Auth Login] onError triggered");
